...
function runOnLoad(window, callback, winType) {
  //log("runOnLoad " + winType);
  // Listen for one load event before checking the window type
  window.addEventListener("load", function() {
    window.removeEventListener("load", arguments.callee, false);
    //log("runOnLoad.load " + window.document.documentElement.getAttribute("windowtype"));
...
function runOnWindows(callback, winType) {
  ...
  function watcher(window) {
    if (!window) {
      log("runOnWindows.watcher error: window is not defined");
    }

    try {
      callback(window);
    } catch(ex) {
      log("runOnWindows.watcher exception: " + ex);
    }
  }
...
function watchWindows(callback, winType) {
  ...
  function watcher(window) {
    try {
      ...
      log("watchWindows.watcher exception: " + ex);
    }
  }

  //log("watchWindows " + winType);
  ...
  function windowWatcher(subject, topic) {
    //log("windowWatcher topic=" + topic + " winType=" + winType)
